...
packages()
{
  ...
  cat "${cwd}/settings/packages.common" | sed '/^#/d' | xargs /usr/local/sbin/pkg-static -c "${uzip}" install -y
  while read -r p; do
  ...
  # cat "${cwd}/settings/packages.common" | sed '/^#/d' | xargs /usr/local/sbin/pkg-static -c "${uzip}" info -d
  # cat "${cwd}/settings/packages.${desktop}" | sed '/^#/d' | xargs /usr/local/sbin/pkg-static -c "${uzip}" info -d
  cat "${cwd}/settings/packages.${desktop}" | sed '/^#/d' | xargs /usr/local/sbin/pkg-static -c "${uzip}" install -y
